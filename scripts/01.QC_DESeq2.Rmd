...
```{r}
umipersample <- colSums(counts$umicount$inex$all) %>% as.data.frame() %>%
  rownames_to_column("barcode") %>%
  dplyr::rename("UMI" = ".")
```

```{r}
umipersample <- left_join(metadata, umipersample, by = "barcode") %>%
  mutate(sample = fct_reorder(sample, UMI, .desc = TRUE))
```

```{r}
y_max <- max(umipersample$UMI)
y_min <- min(umipersample$UMI)
y_lim <- y_max * 1.1
sample_number <- length(umipersample$sample)

# Create the bar plot
ggplot(umipersample, aes(x = sample, y = UMI, fill = Group)) +
  geom_bar(stat = "identity") +
  geom_hline(yintercept = y_max, linetype = "dashed", size = 0.25) +
  geom_hline(yintercept = y_min, linetype = "dashed", size = 0.25) +
  annotate("text", x = 1, y = y_max, label = paste("Max:", scales::comma(y_max)),
  ...
  annotate("text", x = sample_number, y = y_min, label = paste("Min:", scales::comma(y_min)), 
           vjust = 1.5, hjust = 1, size = 6 / ggplot2:::.pt) +
  labs(x = "Sample", y = "UMIs", title = "UMIs per Sample by Group") +
  ggplot2::scale_fill_manual(values = palette_groups) +
  gg_theme() + 
  theme(
    axis.text.x = ggtext::element_markdown(angle = 45, hjust = 1),
...
annotation <- biomaRt::getBM(
  ...
  mart = useEnsembl(
    ...
    mirror = "useast", # "useast", "uswest","asia", "www"
...
smallestGroupSize <- dds.filter.sample@colData %>%
  as.data.frame() %>%
...
```{r}
excluded.genes <- unique(c(
  mito_genes$ensembl_gene_id,        # mitochondrial genes
  rRNA_genes$ensembl_gene_id        # rRNA genes
  # RiboRelated_genes$ensembl_gene_id  # ribosome-related genes
))
